...
from dlvidu import *    # absolute import in main module
...
def cli_main():
    ...
    parser.add_argument("-v", action="count", dest="verbose_lvl", default=0,
        help="Upto four levels (-vvvv): waring, info, debug, details. If not given, default error level")
    parser.add_argument("-l", action="store_true", dest="list_only", default=False, help="Just list video info")
    parser.add_argument("-r", dest="rate_limit", metavar="<BitPerSec>",
        help="Capped maxmium downlad speed")
    parser.add_argument("--buffer-size", dest="buffer_size", metavar="<#>", default="1024",
        help="Size of download buffer (e.g. 1024 or 16384) (default=%(default)s)")
    parser.add_argument("--http-chunk-size", dest="http_chunk_size", metavar="<#>", default=None,
        help="Size of a chunk for chunk-based HTTP downloading when webserver throttles (e.g. 10485760 for 10M) (default is disabled).")
